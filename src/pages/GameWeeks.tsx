...
const GameWeeksPage = () => {
  ...
  const fetchData = async () => {
    ...
    if (gwRes.error) toast({ title: t("error"), description: gwRes.error.message, variant: "destructive" });
  ...
  const handleDelete = async (g: GameWeek) => {
    if (!confirm(t("deleteGameWeek"))) return;
  ...
  const columns: Column<GameWeek>[] = [
    { key: "number", label: t("weekNumber") },
    { key: "season_id", label: t("season"), render: (g) => `${t("season")} ${g.seasons?.number ?? "?"}` },
  ...
  return (
    <>
      <DataTable title={t("gameweeks")} columns={columns} data={data} onAdd={openAdd} onEdit={openEdit} onDelete={handleDelete} loading={loading} />
      <FormDialog open={dialogOpen} onOpenChange={setDialogOpen} title={editing ? t("editGameWeek") : t("addGameWeek")} onSubmit={handleSubmit} loading={saving}>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-2">
            <Label>{t("weekNum")}</Label>
          ...
          <div className="space-y-2">
            ...
            <Select value={form.season_id} onValueChange={v => setForm(f => ({ ...f, season_id: v }))} required>
              ...
              <SelectContent>{seasons.map(s => <SelectItem key={s.id} value={s.id}>{t("season")} {s.number}</SelectItem>)}</SelectContent>
            </Select>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
