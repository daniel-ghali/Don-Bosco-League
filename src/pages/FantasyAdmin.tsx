...
// const FantasyAdmin = () => {
//   const [fantasyTeams, setFantasyTeams] = useState<unknown[]>([]);
//   const [leaderboard, setLeaderboard] = useState<unknown[]>([]);
//   const [chipUsage, setChipUsage] = useState<unknown[]>([]);
...
//     (allPoints || []).forEach((p: { fantasy_team_id: string; points: number }) => { pointsMap[p.fantasy_team_id] = (pointsMap[p.fantasy_team_id] || 0) + p.points; });

//     const lb = (lbRes.data || []).map((ft: Record<string, unknown>) => ({
//       ...ft,
//       totalPoints: pointsMap[ft.id as string] || 0,
//     })).sort((a: Record<string, unknown>, b: Record<string, unknown>) => (b.totalPoints as number) - (a.totalPoints as number));
...
//                           {ft.fantasy_team_players.map((ftp: { id: string; players?: { name: string; last_name: string }; is_captain: boolean }) => (
